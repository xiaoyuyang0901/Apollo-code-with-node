...
/***********************************************************************************
 * 
 * 这一部分是建模的 Zhang Yajia 老师的方法
 * 
 * 起始可以看作一种MPC方法，只不过变量的等式约束（或状态方程）使用的是多项式曲线（也是jerk恒定的？？？3次？？？）
 * 
 * *********************************************************************************/
...
// 设置 决策变量、约束条件、雅可比矩阵非零元素、海森矩阵非零元素 的 个数
...
bool PiecewiseJerkPathIpoptSolver::eval_g(int n, const double* x, bool new_x,
  ...
  // jerk 的不等式约束条件 [-dddx_max_, -dddx_max_]
  // l'''(i_i-1) = [ l''(i) - l''(i-1) ]  / Δs
  for (int i = 0; i + 1 < num_of_points_; ++i) {
  ...
  for (int i = 0; i + 1 < num_of_points_; ++i) {
    ...
    //L'的等式约束 l'(i+1) = l'(i) + l''(i)*Δs + 0.5* l'''(i) * Δs^2
    auto v_diff = v1 - end_v;
    g[num_of_points_ - 1 + i] = v_diff;

    //L的等式约束 l(i+1) = l(i) + l'(i)*Δs + 0.5*l''(i)*Δs^2 + 1/6 *l'''(i) * Δs^3
