...
PiecewiseJerkSpeedNonlinearOptimizer::PiecewiseJerkSpeedNonlinearOptimizer(const TaskConfig& config)
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByNLP(
  ...
  const auto& config = config_.piecewise_jerk_nonlinear_speed_optimizer_config();
  ...
  // 设置warm_start
  // 这一步warm_start设置的就是之前QP优化后的变量结果，
  // 因此NLP是基于QP后的结果再进行的一次优化
  ...
  // 设置s_reference
  // 这一步设置优化变量s的参考值，即QP优化出来的distance向量
  ...
  // 构造接口类的智能指针来存放优化问题的具体形式 ，并调用IPOPT进行求解
