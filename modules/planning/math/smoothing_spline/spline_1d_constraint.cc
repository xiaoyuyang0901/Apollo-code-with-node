...
bool Spline1dConstraint::AddBoundary(const std::vector<double>& x_coord,
  ...
  const uint32_t num_params = spline_order_ + 1;                      //参数的个数
  ...
  // A的列数是(x_knots_.size() - 1) * num_params。每一段curve的列是num_params，
  // inequality_constraint包含所有的curve,因此是(x_knots_.size() - 1) * num_params；x_knots_--
  // 每一段curve起点处s坐标的集合。
  Eigen::MatrixXd inequality_boundary = Eigen::MatrixXd::Zero(
      filtered_upper_bound.size() + filtered_lower_bound.size(), 1);    //初始化矩阵b
...
/***************AddPointConstraint******************
  *****这个函数是添加某个点的函数值约束，我在是使用的时候添加的是每段curve起始点的约束。
  ********************************************/
