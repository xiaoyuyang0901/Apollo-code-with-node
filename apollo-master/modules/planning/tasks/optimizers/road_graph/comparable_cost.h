...
class ComparableCost {
 public:
  ...
  int CompareTo(const ComparableCost &other) const {
    //依次比较has_collision、out_of_boundary、out_of_lane，
    // 谁不正常，谁的cost就大
    //都不正常/正常，跳过进而比较smoothness_cost_和safety_cost_
    for (size_t i = 0; i < cost_items.size(); ++i) {
      if (cost_items[i]) {
        if (other.cost_items[i]) {
          continue;               //this为true，other为true
        } else {
          return 1;               //this为true，other为false
        }
      } else {
        if (other.cost_items[i]) {
          return -1;              //this为false，other为true
        } else {
          continue;               //this为false，other为false
        }
      }
    }
